...
import type { KPIData, VisiteChartData, PatientAlerte, VisiteWithPatient, Patient, PendingAdmin, PharmacyStats } from '@/types';
...
export default function AdminDashboardPage() {
  const { profile, loading, token, logout } = useRequireAuth(['admin', 'superadmin']);
  const isSuperAdmin = profile?.role === 'superadmin';
  const [kpis, setKpis] = useState<KPIData>({ total_patients: 0, patients_chroniques: 0, visites_jour: 0, taux_retour: 0 });
  ...
  const [patientSegmentFilter, setPatientSegmentFilter] = useState<string>('all');
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [
        ...
        supabase.from('patients').select('*', { count: 'exact', head: true }).eq('segment_ia', 'Chronique'),
        supabase.from('visites').select('*', { count: 'exact', head: true }).gte('date', `${today}T00:00:00`),
        supabase.from('patients').select('*').order('created_at', { ascending: false }),
        supabase.from('visites').select('*, patients(name, phone, segment_ia)').order('date', { ascending: false }).limit(5),
        supabase.from('patients').select('*').in('segment_ia', ['Chronique', 'Risque']),
      ]);

      // Calculate taux_retour from already loaded patients (no extra queries)
      let tauxRetour = 0;
      if (patientsData && patientsData.length > 0) {
        const sampleIds = patientsData.slice(0, 50).map((p: { id: string }) => p.id);
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const count = weekVisites?.filter((v: { date: string }) => v.date.startsWith(dateStr)).length || 0;
        chart.push({
          ...
          if (lastDate) {
            ...
            if (daysSince > 30) {
              alertList.push({
                ...
                segment_ia: p.segment_ia, derniere_visite: lastDate, jours_sans_visite: daysSince,
              });
            }
          }
        }
          ...
          for (const p of allProfiles) {
            ...
            if (!pharmacyMap.has(name)) {
              pharmacyMap.set(name, {
                pharmacy_id: p.id, pharmacy_name: name,
                total_patients: 0, total_admins: 0, total_assistants: 0,
              });
            }
  ...
  if (loading || !profile || dataLoading) {
    return (
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, #F8F9FA 0%, #EDF5F0 50%, #F0FAF4 100%)',
        }}
      >
        <div className="flex flex-col items-center gap-4">
          <Spinner size="lg" />
          <p className="text-sm font-medium text-[#6C757D]">Chargement du tableau de bord...</p>
        </div>
      </div>
    );
  }
  ...
  if (dataError) {
    return (
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, #F8F9FA 0%, #EDF5F0 40%, #F0FAF4 70%, #F8F9FA 100%)',
        }}
      >
    ...
    >
      ...
      <main className="lg:ml-[272px] p-6 lg:p-8 relative z-10">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-[#6C757D] mt-1">
                Bienvenue, <span className="font-semibold text-[#2D6A4F]">{profile.email || profile.phone}</span>
              </p>
            </div>
            <div className="hidden md:flex items-center gap-2 text-xs text-[#6C757D]">
              <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="font-medium">Systeme actif</span>
            </div>
          </div>
        </motion.div>
            ...
            <Link key={link.href} href={link.href}>
              ...
              >
                <div
                  ...
                  style={{
                    background: 'linear-gradient(135deg, rgba(45,106,79,0.08) 0%, rgba(82,183,136,0.12) 100%)',
                    color: '#2D6A4F',
                  }}
                >
                  {link.icon}
                ...
                <div>
                  <p className="text-sm font-bold text-[#1A1A2E]">{link.label}</p>
                  <p className="text-[10px] text-[#6C757D] font-medium">{link.description}</p>
                </div>
                <svg className="ml-auto text-[#9CA3AF]" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <AlertesPanel alertes={alertes} pharmacyName={profile.pharmacy_name || 'Pharmacie FATIMA'} />
        </div>
        ...
        >
          <div className="px-6 pt-6 pb-4 flex items-center justify-between border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
            <div className="flex items-center gap-3">
              <div className="p-2.5 rounded-xl" style={{ background: 'linear-gradient(135deg, rgba(45,106,79,0.08) 0%, rgba(82,183,136,0.12) 100%)' }}>
              ...
              <div>
                <h3 className="text-sm font-bold text-[#1A1A2E]">Base de donnees patients</h3>
                <p className="text-[10px] text-[#6C757D] font-medium mt-0.5">{patients.length} patient{patients.length > 1 ? 's' : ''} en base</p>
              </div>
            </div>
            <Link href="/dashboard/admin/patients" className="text-xs font-bold flex items-center gap-1 transition-colors duration-200" style={{ color: '#2D6A4F' }}>
              Voir details
          ...
          {/* Filters */}
          <div className="px-6 py-3 flex flex-col sm:flex-row items-start sm:items-center gap-3 border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              {['all', 'Chronique', 'Risque', 'Suivi régulier', 'Occasionnel'].map((seg) => {
                const isActive = patientSegmentFilter === seg;
                const segStyle = seg === 'all' ? { color: '#2D6A4F', bg: 'rgba(45,106,79,0.08)' } : getSegmentStyle(seg);
                return (
                  <button
                    key={seg}
                    ...
                    style={{
                      ...
                      borderColor: isActive ? segStyle.color + '40' : 'transparent',
                    }}
                  >
                    {seg === 'all' ? 'Tous' : seg}
                  </button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
                  <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Patient</th>
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Telephone</th>
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Segment IA</th>
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Score Fidelite</th>
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Dernier motif</th>
                  <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-widest text-[#6C757D]">Date creation</th>
                </tr>
              </thead>
              <tbody>
                {(() => {
                  const filtered = patients.filter((p) => {
                    const matchSearch = patientSearch === '' ||
                      (p.name || '').toLowerCase().includes(patientSearch.toLowerCase()) ||
                      p.phone.includes(patientSearch);
                    const matchSegment = patientSegmentFilter === 'all' || p.segment_ia === patientSegmentFilter;
                  ...
                  if (filtered.length === 0) {
                    return (
                      <tr>
                        <td colSpan={6} className="px-6 py-10 text-center text-sm text-[#6C757D]">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs text-[#6C757D] font-mono">{patient.phone}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span
                          ...
                          >
                            <span className="w-1.5 h-1.5 rounded-full" style={{ background: segStyle.color }} />
                            ...
                            <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  ...
                                  background: patient.score_fidelite > 70 ? 'linear-gradient(90deg, #2D6A4F, #52B788)' : patient.score_fidelite > 40 ? 'linear-gradient(90deg, #40916C, #74C69D)' : 'linear-gradient(90deg, #9CA3AF, #D1D5DB)',
                                }}
                              />
                            </div>
                            <span className="text-xs font-bold" style={{ color: patient.score_fidelite > 70 ? '#2D6A4F' : patient.score_fidelite > 40 ? '#40916C' : '#9CA3AF' }}>
            ...
            {patients.filter((p) => {
              ...
              <div className="px-6 py-3 text-center border-t" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
                <Link href="/dashboard/admin/patients" className="text-xs font-bold" style={{ color: '#2D6A4F' }}>
                  Voir les {patients.filter((p) => {
                    const matchSearch = patientSearch === '' ||
                      (p.name || '').toLowerCase().includes(patientSearch.toLowerCase()) ||
                      p.phone.includes(patientSearch);
                    const matchSegment = patientSegmentFilter === 'all' || p.segment_ia === patientSegmentFilter;
          ...
          <div className="px-6 pt-6 pb-4 flex items-center justify-between border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ background: 'linear-gradient(135deg, rgba(45,106,79,0.08) 0%, rgba(82,183,136,0.12) 100%)' }}
              >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2D6A4F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div>
                <h3 className="text-sm font-bold text-[#1A1A2E]">Dernieres visites</h3>
                <p className="text-[10px] text-[#6C757D] font-medium mt-0.5">Activite recente</p>
              </div>
            </div>
            <Link
              href="/dashboard/admin/patients"
          ...
          <div className="p-4">
            {recentVisites.length === 0 ? (
              <div className="text-center py-10">
                <p className="text-sm text-[#6C757D]">Aucune visite enregistree.</p>
              </div>
            ) : (
              <div className="space-y-2">
                {recentVisites.map((visite, index) => {
                  const segStyle = getSegmentStyle(visite.patients?.segment_ia || null);
                  return (
                    <motion.div
                      key={visite.id}
                      ...
                      onMouseEnter={(e) => {
                        (e.currentTarget as HTMLDivElement).style.background = 'rgba(45,106,79,0.03)';
                      }}
                      onMouseLeave={(e) => {
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{ background: getAvatarGradient(visite.patients?.name || null) }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[#6C757D] truncate mt-0.5">{visite.motif}</p>
                      </div>

                      {/* Tag and Date */}
                      <div className="flex items-center gap-3 flex-shrink-0">
                        {visite.tag && (
                          <Badge variant={visite.tag === 'Chronique' ? 'green' : visite.tag === 'Aigu' ? 'orange' : 'blue'}>
                            {visite.tag}
                          </Badge>
                        )}
                        <span className="text-[11px] text-[#9CA3AF] font-medium whitespace-nowrap">
        ...
        {isSuperAdmin && (
          ...
          >
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-extrabold text-[#1A1A2E]">Administration globale</h2>
                <p className="text-xs text-[#6C757D]">Gestion des pharmacies et des demandes d&apos;acces</p>
              </div>
            </div>
