...
function getSegmentStyle(segment: string | null) {
  switch (segment) {
    case 'Chronique':
      return { color: '#2D6A4F', bg: 'rgba(45,106,79,0.1)', border: 'rgba(45,106,79,0.2)', label: 'Chronique' };
    case 'Risque':
      return { color: '#DC2626', bg: 'rgba(220,38,38,0.08)', border: 'rgba(220,38,38,0.15)', label: 'Risque' };
    case 'Suivi régulier':
      return { color: '#2563EB', bg: 'rgba(37,99,235,0.08)', border: 'rgba(37,99,235,0.15)', label: 'Suivi regulier' };
    case 'Occasionnel':
      return { color: '#6C757D', bg: 'rgba(108,117,125,0.08)', border: 'rgba(108,117,125,0.15)', label: 'Occasionnel' };
    default:
      return { color: '#9CA3AF', bg: 'rgba(108,117,125,0.06)', border: 'rgba(108,117,125,0.1)', label: 'Non classe' };
  }
}
...
export default function PatientsPage() {
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <Spinner size="lg" />
          <p className="text-sm font-medium text-[#6C757D]">Chargement des patients...</p>
        </div>
      </div>
    );
  }
  ...
  const segmentCounts = {
    ...
    'Suivi régulier': patients.filter((p) => p.segment_ia === 'Suivi régulier').length,
    Occasionnel: patients.filter((p) => p.segment_ia === 'Occasionnel').length,
  };

  const selectedSegStyle = selectedPatient ? getSegmentStyle(selectedPatient.segment_ia) : null;
      ...
      <main className="lg:ml-[272px] p-6 lg:p-8 relative z-10">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-extrabold text-[#1A1A2E] tracking-tight">Patients</h1>
              <p className="text-sm text-[#6C757D] mt-1">
                <span className="font-bold text-[#2D6A4F]">{patients.length}</span> patient{patients.length > 1 ? 's' : ''} enregistre{patients.length > 1 ? 's' : ''}
              </p>
            </div>

            {/* Segment summary pills */}
            <div className="hidden lg:flex items-center gap-2">
              {(['Chronique', 'Risque', 'Suivi régulier', 'Occasionnel'] as const).map((seg) => {
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-[#6C757D] mt-0.5">{selectedPatient.phone}</p>
                    <div className="flex items-center gap-2 mt-2">
                      {selectedSegStyle && (
                        ...
                        >
                          ...
                          Segment IA: {selectedPatient.segment_ia || 'Non classe'}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[#6C757D] mb-1">Score Fidelite</p>
                    <div className="flex items-center gap-3">
                      <div className="flex-1">
                        <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${selectedPatient.score_fidelite}%` }}
                            transition={{ duration: 0.8, ease: 'easeOut' }}
                            className="h-full rounded-full"
                            style={{
                              background: `linear-gradient(90deg, #2D6A4F, ${
                                selectedPatient.score_fidelite > 70 ? '#52B788' : '#40916C'
                              })`,
                            }}
                          />
                        </div>
                      </div>
                      <span className="text-lg font-extrabold" style={{ color: '#2D6A4F' }}>
                  ...
                  >
                    <p className="text-[10px] font-bold uppercase tracking-widest text-[#6C757D] mb-1">Enregistre le</p>
                    <p className="text-lg font-extrabold text-[#1A1A2E]">{formatDate(selectedPatient.created_at)}</p>
                  </div>
                </div>

                {/* WhatsApp Action */}
                <a
                  href={generateWhatsAppUrl(selectedPatient.phone, selectedPatient.name || '', profile.pharmacy_name || 'Pharmacie FATIMA')}
                  target="_blank"
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-sm font-bold text-[#1A1A2E]">Historique des visites</h4>
                    {patientVisites.length > 0 && (
                      <span className="text-[10px] font-bold text-[#6C757D] ml-auto">
                        {patientVisites.length} visite{patientVisites.length > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                  {patientVisites.length === 0 ? (
                    ...
                    <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(45,106,79,0.2) transparent' }}>
                      {patientVisites.map((visite, index) => (
                        <motion.div
                          ...
                          onMouseEnter={(e) => {
                            (e.currentTarget as HTMLDivElement).style.background = 'rgba(45,106,79,0.05)';
                          }}
                          onMouseLeave={(e) => {
                            (e.currentTarget as HTMLDivElement).style.background = 'rgba(45,106,79,0.02)';
                          }}
                        >
                          {/* Timeline dot */}
                            ...
                            <div
                              ...
                              style={{
                                background: index === 0
                                  ? 'linear-gradient(135deg, #2D6A4F, #52B788)'
                                  : 'rgba(45,106,79,0.2)',
                                boxShadow: index === 0 ? '0 0 8px rgba(45,106,79,0.3)' : 'none',
                              }}
                            />
                          </div>
                          ...
                          {/* Content */}
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-[#1A1A2E] truncate">{visite.motif}</p>
                          </div>

                          {/* Tag and Date */}
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {visite.tag && (
                              <Badge variant={visite.tag === 'Chronique' ? 'green' : visite.tag === 'Aigu' ? 'orange' : 'blue'} size="sm">
                                {visite.tag}
                              </Badge>
                            )}
