...
function getSeverity(days: number) {
  if (days > 90) {
    return { label: 'Critique', color: '#DC2626', bg: 'rgba(220,38,38,0.06)', border: 'rgba(220,38,38,0.12)', ring: 'rgba(220,38,38,0.15)' };
  }
  if (days > 60) {
    return { label: 'Urgent', color: '#EA580C', bg: 'rgba(234,88,12,0.06)', border: 'rgba(234,88,12,0.12)', ring: 'rgba(234,88,12,0.15)' };
  }
  return { label: 'Attention', color: '#D97706', bg: 'rgba(217,119,6,0.06)', border: 'rgba(217,119,6,0.12)', ring: 'rgba(217,119,6,0.15)' };
}

function getSegmentStyle(segment: string | null) {
  switch (segment) {
    case 'Chronique':
      return { color: '#2D6A4F', bg: 'rgba(45,106,79,0.1)', border: 'rgba(45,106,79,0.2)' };
    case 'Risque':
      return { color: '#DC2626', bg: 'rgba(220,38,38,0.08)', border: 'rgba(220,38,38,0.15)' };
    case 'Suivi régulier':
      return { color: '#2563EB', bg: 'rgba(37,99,235,0.08)', border: 'rgba(37,99,235,0.15)' };
    default:
      return { color: '#6C757D', bg: 'rgba(108,117,125,0.08)', border: 'rgba(108,117,125,0.15)' };
  }
}
...
export default function RelancesPage() {
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-4">
          <Spinner size="lg" />
          <p className="text-sm font-medium text-[#6C757D]">Chargement des relances...</p>
        </div>
      </div>
    );
  }
  ...
  const urgentCount = alertes.filter((a) => a.jours_sans_visite > 60 && a.jours_sans_visite <= 90).length;
      ...
      <main className="lg:ml-[272px] p-6 lg:p-8 relative z-10">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-extrabold text-[#1A1A2E] tracking-tight">Relances</h1>
              <p className="text-sm text-[#6C757D] mt-1">
                Patients a relancer ({'>'}30 jours sans visite)
              </p>
            </div>
            {alertes.length > 0 && (
              <div className="hidden md:flex items-center gap-2">
                {criticalCount > 0 && (
                  <div
                    ...
                    style={{ background: 'rgba(220,38,38,0.06)', color: '#DC2626', borderColor: 'rgba(220,38,38,0.12)' }}
                  >
                ...
                {urgentCount > 0 && (
                  <div
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border"
                    style={{ background: 'rgba(234,88,12,0.06)', color: '#EA580C', borderColor: 'rgba(234,88,12,0.12)' }}
                  >
                ...
                {warningCount > 0 && (
                  <div
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border"
                    style={{ background: 'rgba(217,119,6,0.06)', color: '#D97706', borderColor: 'rgba(217,119,6,0.12)' }}
                  >
        ...
        {alertes.length === 0 ? (
          ...
          >
            <div className="text-center py-20 px-6">
              ...
              >
                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#2D6A4F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M20 6L9 17l-5-5" />
                </svg>
              </motion.div>
              <h2 className="text-xl font-extrabold text-[#1A1A2E] mb-2">Tout est a jour</h2>
              <p className="text-sm text-[#6C757D] max-w-sm mx-auto">
                Aucun patient ne necessite de relance pour le moment. Tous les patients sont suivis regulierement.
              </p>
            </div>
          </motion.div>
        ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span
                            ...
                            style={{ background: segStyle.bg, color: segStyle.color, borderColor: segStyle.border }}
                          >
                            <span className="w-1 h-1 rounded-full" style={{ background: segStyle.color }} />
                            {alerte.segment_ia}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-[#6C757D]">
                          ...
                          <span>Derniere visite: {formatDate(alerte.derniere_visite)}</span>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          {/* WhatsApp button */}
                          <a
                            href={generateWhatsAppUrl(alerte.phone, alerte.name || '', pharmacyName)}
                            target="_blank"
