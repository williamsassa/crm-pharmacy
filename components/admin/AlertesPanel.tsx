...
function getSeverity(days: number): { label: string; color: string; bg: string; border: string; glow: string } {
  if (days > 90) {
    return {
      label: 'Critique',
      color: '#DC2626',
...
export default function AlertesPanel({ alertes, pharmacyName = 'Pharmacie FATIMA' }: AlertesPanelProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="px-6 pt-6 pb-4 flex items-center justify-between border-b" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: alertes.length > 0 ? 'rgba(220,38,38,0.08)' : 'rgba(45,106,79,0.08)' }}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={alertes.length > 0 ? '#DC2626' : '#2D6A4F'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div>
            <h3 className="text-sm font-bold text-[#1A1A2E]">Alertes de relance</h3>
            <p className="text-[10px] text-[#6C757D] font-medium mt-0.5">Patients inactifs</p>
          </div>
        </div>
        {alertes.length > 0 && (
          <div
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3">
        {alertes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="text-sm font-semibold text-[#1A1A2E]">Tout est a jour</p>
            <p className="text-xs text-[#6C757D] mt-1">Aucun patient a relancer</p>
          </div>
        ) : (
                  ...
                  <div className="pl-2.5">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[#6C757D] mb-1.5">{alerte.phone}</p>
                        <div className="flex items-center gap-2">
                          <span
                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold"
                            style={{ background: severity.glow, color: severity.color }}
                          >
                          ...
                          <span className="text-[10px] text-[#9CA3AF]">{severity.label}</span>
                        </div>
                      </div>
      ...
      {alertes.length > 5 && (
        <div className="px-6 py-3 border-t" style={{ borderColor: 'rgba(0,0,0,0.04)' }}>
          <a
