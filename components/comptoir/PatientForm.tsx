...
export default function PatientForm({ onSuccess }: PatientFormProps) {
  ...
  const searchPatient = useCallback(async (phoneValue: string) => {
    ...
    try {
      const res = await fetch(`/api/patients/list?phone=${encodeURIComponent(phoneValue)}`, {
  ...
  return (
    ...
    >
      ...
      >
        <label
          htmlFor="phone-input"
          className="block text-sm font-medium text-pharma-text mb-2 flex items-center gap-2"
        >
          ...
          {existingPatient && (
            ...
            >
              <p className="mt-2 text-sm text-pharma-green font-medium flex items-center gap-1.5 pl-1">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-pharma-green-light">
